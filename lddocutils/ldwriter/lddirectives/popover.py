...
class PopoverDirective(Directive):
    ...
    def run(self):
        ...
        container["button_classes"] = self.options.get("class", []) or []
...
def visit_popover(self, node: popover):
    ...
    self.body.append(
        f'<button type="button" class="{btn_classes}" popovertarget="{pop_id}">'
    )
    ...
    self.body.append(f'<dialog id="{pop_id}" popover="auto">')
